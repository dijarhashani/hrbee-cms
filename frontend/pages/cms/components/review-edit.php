...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql = "SELECT name, description, photo_path, position FROM reviews WHERE id = ?";
...
<div class="trajneret">
    <div class="imazhi">
                <div id="image-preview" style="margin-top: 10px;">
                    <?php if (!empty($row['photo_path'])): ?>
                        <img src="frontend/uploads/reviews/<?php echo htmlspecialchars($row['photo_path']); ?>" alt="Program Image" style="max-width: 100%;">
                    <?php endif; ?>
                </div>
    ...
    <div class="name-surname">
        <h3>Emri dhe mbiemri i klientit:</h3>
        <input type="text" name="name-surname" id="name-surname" value="<?php echo htmlspecialchars($row['name'])?>">
    </div>
    <div class="position">
        <h3>Pozita e klientit:</h3>
        <input type="text" name="position" id="position" value="<?php echo htmlspecialchars($row['position']);?>">
    </div>
    <div class="description">
        <h3>Vlerësimet nga klienti:</h3>
...
document.querySelector('button.__btn').addEventListener('click', function(event) {
    ...
    fetch('backend/reviews-update.php', {
        method: 'POST',
        body: formData
    })
...
function deleteForm() {
    ...
    fetch('backend/review-delete.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
