...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql_select = "SELECT photo_path FROM reviews WHERE id = ?";
    ...
    $sql = "UPDATE reviews SET name = ?, description = ?, position = ?, photo_path = ? WHERE id = ?";
    ...
    if ($stmt->affected_rows > 0) {
        echo json_encode(['success' => 'Vleresimi u ndryshua me sukses.']);
    } else {
        echo json_encode(['error' => 'Asnje ndryshim nuk u be. Ju lutem ndryshoni Vleresimin.']);
    }
